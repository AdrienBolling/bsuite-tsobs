...
class TimeSeriesObservation(dm_env.Environment):
    """
  Environment wrapper to convert observations to a time-series format.
  A time-series observation is a sequence of observations.
  """

    def __init__(self, env: dm_env.Environment):
        self._env = env
        empty_observation = env.reset().observation
        self._max_episode_length = env._max_steps if hasattr(env, '_max_steps') else 1000
        self._empty_observation = empty_observation.flatten()

        # Initialize the time-series observation.
        self._time_series_observation = np.tile(empty_observation, (self._max_episode_length, 1))

        # Initialize the time-series observation index.
        self._time_series_observation_index = 0

    def observation_spec(self):
        spec = self._env.observation_spec()
        # Reflect the flattening of the observation in the raw environment, multiply all dimensions of the spec
        _flattened_spec_shape = reduce(lambda x, y: x * y, spec.shape, 1)
        return specs.Array(shape=(self._max_episode_length, _flattened_spec_shape), dtype=spec.dtype, name=spec.name)
    ...
    def reset(self):
        timestep = self._env.reset()
        self._time_series_observation_index = 0
        self._time_series_observation = np.tile(self._empty_observation, (self._max_episode_length, 1))
        return timestep._replace(observation=self._time_series_observation)

    def step(self, action):
        timestep = self._env.step(action)
        flattened_obs = timestep.observation.flatten()
        self._time_series_observation[self._time_series_observation_index] = flattened_obs
        self._time_series_observation_index += 1

        # Check if we are at the end of the episode.
        if self._max_episode_length == self._time_series_observation_index:
            timestep = timestep._replace(step_type=dm_env.StepType.LAST)
            return dm_env.termination(reward=timestep.reward, observation=self._time_series_observation)

        return timestep._replace(observation=self._time_series_observation)

    def raw_env(self):
        """
        Contrary to the other wrappers, this wrapper is used as a permanent alteration to the environment.
        Therefore, the raw_env method returns the wrapped environment itself.
        """
        return self
